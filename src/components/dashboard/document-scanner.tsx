...
export function DocumentScanner() {
      ...
      if (files[0].type.startsWith("image/")) {
        if (files.length > 1) {
          toast({
            ...
            description:
              "For multi-page documents, only the first page will be processed and saved.",
          });
        }
        setFileType("image");
  ...
  const getCardTitle = () => {
    ...
    switch (scannerState) {
      case "idle":
        return (
          <>
            <UploadCloud /> Hello {name}! Ready to file?
          </>
        );
      case "capturing":
        return (
          <>
            <FileEdit /> Document ready for processing.
          </>
        );
      case "processing":
        return (
          <>
            <Loader2 className="animate-spin" /> Lia is processing your
            document...
          </>
        );
      case "reviewing":
        return (
          <>
            <Sparkles /> Here's what Lia found.
          </>
        );
      case "saving":
        return (
          <>
            <Loader2 className="animate-spin" /> Filing your document...
          </>
        );
    }
  };
  ...
  const getCardDescription = () => {
    switch (scannerState) {
      case "idle":
        return "Click the button below to upload a document.";
      case "capturing": {
        if (fileType === "pdf")
      ...
      case "saving":
        return "Filing your document securely in the Smart Mailbox...";
    }
  };
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  id="filename"
                  name="filename"
                  defaultValue={aiResult.filename}
              ...
              <div>
                ...
                <Input
                  id="folderPath"
                  name="folderPath"
                  defaultValue={aiResult.folderPath}
                />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                {aiResult.metadata.sender && (
                  <p>
                    <strong className="text-muted-foreground">Sender:</strong>{" "}
                    {aiResult.metadata.sender}
                  </p>
                )}
                {aiResult.metadata.date && (
                  <p>
                    <strong className="text-muted-foreground">Date:</strong>{" "}
                    {aiResult.metadata.date}
                  </p>
                )}
                {aiResult.metadata.category && (
                  <p>
